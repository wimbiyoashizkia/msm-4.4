...
setversioning() {
	KERNELNAME="$KERNEL-$KERNELTYPE-$TYPE-$DEVICE-$DATE"
...
build_kernel() {
	...
	make clean && make mrproper && rm -rf out && make O=out $DEFCONFIG
...
gen_zip() {
	mv "$KERNEL_DIR"/out/arch/arm64/boot/Image.gz-dtb AnyKernel3/X01BD/Image.gz-dtb
	cd AnyKernel3/X01BD || exit
