...
resched:
	queue_delayed_work(system_power_efficient_wq,
		&chip->check_sanity_work, msecs_to_jiffies(SANITY_CHECK_PERIOD_MS));
}
...
out:
	if (!rc)
		queue_delayed_work(system_power_efficient_wq,
			&chip->update_sram_data, msecs_to_jiffies(resched_ms));
}
...
resched:
	queue_delayed_work(system_power_efficient_wq,
		&chip->update_temp_work, msecs_to_jiffies(TEMP_PERIOD_UPDATE_MS));
}
...
static void status_change_work(struct work_struct *work)
{
	...
	if (chip->prev_status != chip->status && chip->last_sram_update_time) {
		...
		if (chip->last_temp_update_time && chip->soc_slope_limiter_en) {
			...
			queue_delayed_work(system_power_efficient_wq,
				&chip->update_temp_work, msecs_to_jiffies(0));
		}
...
reschedule:
	queue_delayed_work(system_power_efficient_wq,
		&chip->batt_profile_init, msecs_to_jiffies(BATTERY_PSY_WAIT_MS));
	return 0;
}
