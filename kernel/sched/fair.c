...
static inline u32 binary_smooth(u32 new, u32 old)
{
	...
	return (0 <= increment) ? old + (increment >> sched_burst_smoothness_up) : old - (-increment >> sched_burst_smoothness_down);
}
...
#endif
{
	...
	if (likely(sched_bore))
		delta = penalty_scale_mayhalve(delta, se, half);
#endif
