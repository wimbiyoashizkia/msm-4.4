...
#ifndef HQ_BUILD_FACTORY
		/* reg=1370 */
		rc = smblib_read(smbchg_dev, USBIN_CURRENT_LIMIT_CFG_REG, &USBIN_1_cc);
		if (rc < 0)
			printk("%s: Couldn't read fast_CURRENT_LIMIT_CFG_REG\n", __func__);
#endif
			set_icl = ICL_1500mA;
...
void asus_insertion_initial_settings(struct smb_charger *chg)
{
	...
	/* reg=1051 01000000 bit6=Charge Enable Polarity, 1 = Active low (0: enable charging) */
	...
	/* reg=1042 0 disenable CHARGING_ENABLE_CMD */
	rc = smblib_masked_write(chg, CHARGING_ENABLE_CMD_REG,
	...
	/* reg=1683 vsy_min=3.6v Minimum system voltage setting */
	...
	/* reg=1365 bit4=1 Use SW to control Input Current Limit after APSD is completed */
	rc = smblib_masked_write(chg, USBIN_LOAD_CFG_REG,
