...
uint16_t ArraysBusyFlags;                       /* Binary Flags for Busy Arrays */
uint64_t tm_seed;
int arraysBufferPosition;                       /* Array reserved to determine which buffer to use */
struct TIMESPEC tsp;
                      ...
                      size_t receivedCount, loff_t *ppos)
{
        ...
        pr_info("COPT_FROM_USER receivedCount:%zu\n", receivedCount);
...
void update_sarray(int arraysPosition)
{
        ...
        pr_info("arraysPosition:%d, X:%llu, Y:%llu, Z1:%llu, Z2:%llu, Z3:%llu,\n",
...
void update_sarray_uhs(int arraysPosition)
{
        ...
        pr_info("arraysPosition:%d, X:%llu, Z1:%llu\n", arraysPosition, X, Z1);
}
...
int nextbuffer(void)
{
        ...
        pr_info("raw:%lld", "position:%d", "roll:%d", "%s:%d", "arraysBufferPosition:%d\n",
                 prngArrays[numberOfRndArrays][position], position, roll, __func__,
                 nextbuffer, arraysBufferPosition);
