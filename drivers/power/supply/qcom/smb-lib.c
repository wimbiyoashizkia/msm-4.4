...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		queue_delayed_work(system_power_efficient_wq,
			&chg->pl_enable_work, msecs_to_jiffies(PL_DELAY_MS));
	} else {
		if (chg->wa_flags & BOOST_BACK_WA) {
			data = chg->irq_info[SWITCH_POWER_OK_IRQ].irq_data;
			if (data) {
...
irqreturn_t smblib_handle_icl_change(int irq, void *data)
{
	...
	if (chg->mode == PARALLEL_MASTER) {
		...
		queue_delayed_work(system_power_efficient_wq,
			&chg->icl_change_work, msecs_to_jiffies(delay));
	}
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	case DCP_CHARGER_BIT:
		if (chg->wa_flags & QC_CHARGER_DETECTION_WA_BIT)
			queue_delayed_work(system_power_efficient_wq,
				&chg->hvdcp_detect_work,  msecs_to_jiffies(HVDCP_DET_MS));
		break;
	default:
		break;
	}
...
irqreturn_t smblib_handle_usb_typec_change(int irq, void *data)
{
	...
	if (chg->micro_usb_mode) {
		...
		queue_delayed_work(system_power_efficient_wq,
			&chg->uusb_otg_work, msecs_to_jiffies(chg->otg_delay_ms));
		return IRQ_HANDLED;
	}
...
irqreturn_t smblib_handle_switcher_power_ok(int irq, void *data)
{
	...
	if (is_storming(&irq_data->storm_data)) {
		...
		if (!is_client_vote_enabled(chg->usb_icl_votable,
			 ...
			 */
			queue_delayed_work(system_power_efficient_wq,
				&chg->bb_removal_work, msecs_to_jiffies(BOOST_BACK_UNVOTE_DELAY_MS));
		}
	}
