...
static unsigned long gpu_boost_freq;
module_param(gpu_boost_freq, ulong, 0644);

static unsigned long gpu_boost_duration;
module_param(gpu_boost_duration, ulong, 0644);
...
static void gpu_boost_worker(struct work_struct *work)
{
	...
	devfreq->min_freq = gpu_boost_freq;

	gpu_update_devfreq(devfreq);

	schedule_delayed_work(&unboost_work, msecs_to_jiffies(gpu_boost_duration));
}
		...
		unsigned int type, unsigned int code, int value)
{
	...
	if (!gpu_boost_freq || !gpu_boost_duration)
		return;
	...
	if (gpu_boost_running) {
		if (cancel_delayed_work_sync(&unboost_work)) {
			schedule_delayed_work(&unboost_work,
				msecs_to_jiffies(gpu_boost_duration));
			return;
		}
	}
...
static void gpu_input_boost_init(void)
{
	int ret;
	...
	if (ret)
		pr_err(TAG "failed to register gpu_input_boost handler\n");
}
